...
校订版构建入口会基于 `build_web_handbook.py` 重新生成 `web/data.js` 及相关网页图片，同时应用已确认的中文章节术语，并把官方注释开头的修订公告按中文源稿原文纳入网页正文。Markdown 源稿中的目录继续保留在文档中，但网页使用侧栏目录导航，不再把目录重复渲染成正文内容。请勿直接把生成物作为规则源文档编辑。
